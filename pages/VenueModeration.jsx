...
const VenueModeration = () => {
  ...
  const handleModerate = async (id, status) => {
    ...
    try {
      const API_BASE = import.meta.env.VITE_API_BASE || 'http://localhost:5000/api';
      await axios.put(`${API_BASE}/admin/venues/${id}/moderate`, { status });
          ...
          filteredVenues.map((venue) => (
            <div key={venue.id} className="premium-card group overflow-hidden flex flex-col sm:flex-row h-full">
              ...
              <div className="w-full sm:w-56 h-56 sm:h-auto relative">
                <img 
                  src={venue.img ? `${import.meta.env.VITE_API_BASE ? import.meta.env.VITE_API_BASE.replace('/api', '') : 'http://localhost:5000'}${venue.img}` : 'https://images.unsplash.com/photo-1545118531-158a1fa98d25?q=80&w=600&auto=format&fit=crop'} 
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <div className="flex items-center gap-3 p-3 bg-white/5 rounded-2xl border border-white/5">
                    ...
                    {venue.verificationDoc && (
                      <a 
                        href={`${import.meta.env.VITE_API_BASE ? import.meta.env.VITE_API_BASE.replace('/api', '') : 'http://localhost:5000'}${venue.verificationDoc}`} 
